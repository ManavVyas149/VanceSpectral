...
void ToolbarComponent::ToolButton::paintButton(juce::Graphics &g,
                                               bool isHighlighted,
                                               bool isDown) {
...
void ToolbarComponent::ToolButton::drawFreehandIcon(
  ...
  p.cubicTo(x + w * 0.1f, y + h * 0.2f, x + w * 0.6f, y + h * 0.1f,
            x + w * 0.8f, y + h * 0.4f);
  p.cubicTo(x + w * 0.9f, y + h * 0.7f, x + w * 0.5f, y + h * 0.9f,
            x + w * 0.2f, y + h * 0.7f);
  p.closeSubPath();
...
void ToolbarComponent::ToolButton::drawRectangleIcon(
    juce::Graphics &g, juce::Rectangle<float> bounds, juce::Colour iconColour) {
...
void ToolbarComponent::ToolButton::drawEraseIcon(juce::Graphics &g,
...
void ToolbarComponent::ToolButton::drawCopyIcon(juce::Graphics &g,
  ...
  auto r1 = juce::Rectangle<float>(
      bounds.getX(), bounds.getY() + bounds.getHeight() - h, w, h);
  auto r2 = juce::Rectangle<float>(bounds.getX() + bounds.getWidth() - w,
                                   bounds.getY(), w, h);
...
void ToolbarComponent::setSelectedTool(ToolType newTool) {
  activeTool = newTool;
  freehandBtn.setToggleState(activeTool == ToolType::Freehand,
                             juce::dontSendNotification);
  rectSelectBtn.setToggleState(activeTool == ToolType::RectangleSelect,
                               juce::dontSendNotification);
  eraserBtn.setToggleState(activeTool == ToolType::Erase,
                           juce::dontSendNotification);
  copyBtn.setToggleState(activeTool == ToolType::Copy,
                         juce::dontSendNotification);
  repaint();
}
