...
bool SpectrogramComponent::isInterestedInFileDrag(const juce::StringArray& files)
{
    for (auto& file : files)
    {
...
void SpectrogramComponent::filesDropped(const juce::StringArray& files,
                                        int,
                                        int)
{
    dragActive = false;
...
void SpectrogramComponent::mouseDown(const juce::MouseEvent& e)
{
    ...
    float startX =
        area.getX()+area.getWidth()*startPosition;


    float endX =
        area.getX()+area.getWidth()*endPosition;
...
void SpectrogramComponent::mouseDrag(const juce::MouseEvent& e)
{
    ...
    if(draggingEnd)
        ...
        processor.setRegion(startPosition,
                    endPosition);
...
void SpectrogramComponent::drawWaveform(juce::Graphics& g)
{
    ...
    int samplesPerPixel = juce::jmax(1,
        numSamples / area.getWidth());
    ...
    if (loadedFile.existsAsFile())
        {
            g.setColour(juce::Colours::white);
            g.setFont(18.0f);
            g.drawText("Loaded: " + loadedFile.getFileName(), getLocalBounds().reduced(20), juce::Justification::centredBottom);
        }
    if (fileLoaded)
        {
            drawWaveform(g);
