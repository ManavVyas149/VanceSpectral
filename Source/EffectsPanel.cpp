...
const juce::StringArray &EffectsPanel::getNoteDivisionNames() {
  static const juce::StringArray names{"1/16", "1/8T", "1/8", "1/8D",
                                       "1/4",  "1/4D", "1/2"};
  return names;
}
...
double EffectsPanel::getNoteDivisionFactor(int index) {
  ...
  case 2:
    return 0.5; // 1/8 (Default)
  case 3:
    return 0.75; // 1/8 Dotted
  case 4:
    return 1.0; // 1/4
  case 5:
    return 1.5; // 1/4 Dotted
  case 6:
    return 2.0; // 1/2
  default:
    return 0.5;
...
EffectsPanel::EffectZoneComponent::EffectZoneComponent(
    const EffectZoneConfig &cfg, juce::AudioProcessorValueTreeState &apvtsRef,
    ...
    : config(cfg), apvts(apvtsRef), bpmProvider(bpmProviderFn),
      onSelectRequest(onSelectReqFn), onParameterAdjusted(onParamAdjustedFn) {
...
void EffectsPanel::EffectZoneComponent::updateReadout(
...
float EffectsPanel::EffectZoneComponent::getNormalizedParamValue(
...
void EffectsPanel::EffectZoneComponent::setNormalizedParamValue(
    const juce::String &paramID, float normVal) {
  if (auto *param = apvts.getParameter(paramID)) {
    param->setValueNotifyingHost(juce::jlimit(0.0f, 1.0f, normVal));
  }
}

void EffectsPanel::EffectZoneComponent::syncFromAPVTS() { repaint(); }
...
EffectsPanel::EffectZoneComponent::getHeaderBounds() const {
  return juce::Rectangle<float>(2.0f, 3.0f, (float)getWidth() - 4.0f, 20.0f);
}

juce::Rectangle<float>
EffectsPanel::EffectZoneComponent::getMixBarBounds() const {
  return juce::Rectangle<float>(4.0f, 25.0f, (float)getWidth() - 8.0f, 14.0f);
}

juce::Rectangle<float>
EffectsPanel::EffectZoneComponent::getSecondaryControlsBounds() const {
  float topY = 42.0f;
  float botY = (float)getHeight() - 26.0f;
  return juce::Rectangle<float>(4.0f, topY, (float)getWidth() - 8.0f,
                                juce::jmax(0.0f, botY - topY));
}

juce::Rectangle<float>
EffectsPanel::EffectZoneComponent::getReadoutBounds() const {
  return juce::Rectangle<float>(4.0f, (float)getHeight() - 24.0f,
                                (float)getWidth() - 8.0f, 20.0f);
}

juce::Rectangle<float>
EffectsPanel::EffectZoneComponent::getBypassToggleBounds() const {
  auto area = getSecondaryControlsBounds();
  if (config.index == 3) // DELAY: shares top row with Sync mode
  {
    return juce::Rectangle<float>(area.getX(), area.getY() + 2.0f,
                                  (area.getWidth() - 3.0f) * 0.48f, 16.0f);
  }
  return juce::Rectangle<float>(area.getX() + 2.0f, area.getY() + 2.0f,
                                area.getWidth() - 4.0f, 16.0f);
}

juce::Rectangle<float>
EffectsPanel::EffectZoneComponent::getDelaySyncToggleBounds() const {
  auto area = getSecondaryControlsBounds();
...
juce::Rectangle<float>
EffectsPanel::EffectZoneComponent::getDelayDivisionBounds() const {
  auto area = getSecondaryControlsBounds();
  return juce::Rectangle<float>(area.getX() + 2.0f, area.getY() + 22.0f,
                                area.getWidth() - 4.0f, 18.0f);
}
...
void EffectsPanel::EffectZoneComponent::resized() {
  ...
  {
    float knobBoxW = area.getWidth() - 4.0f;
    float knobBoxH = 36.0f;

    if (delayIsSynced) {
      // Secondary knob 2: Feedback (placed below division selector)
      secondaryKnob2.bounds =
          juce::Rectangle<float>(area.getCentreX() - knobBoxW * 0.5f,
                                 area.getY() + 44.0f, knobBoxW, knobBoxH);
      secondaryKnob2.label = "FDBK";
      secondaryKnob2.paramID = "FX_DELAY_FEEDBACK";
    } else {
      // Secondary knob 1: Time (when free)
      secondaryKnob1.bounds =
          juce::Rectangle<float>(area.getCentreX() - knobBoxW * 0.5f,
                                 area.getY() + 22.0f, knobBoxW, knobBoxH);
      ...
      secondaryKnob2.bounds =
          juce::Rectangle<float>(area.getCentreX() - knobBoxW * 0.5f,
                                 area.getY() + 62.0f, knobBoxW, knobBoxH);
      secondaryKnob2.label = "FDBK";
      secondaryKnob2.paramID = "FX_DELAY_FEEDBACK";
    }
  } else {
    float knobBoxW = area.getWidth() - 4.0f;
    float knobBoxH = 46.0f;
    secondaryKnob1.bounds =
        juce::Rectangle<float>(area.getCentreX() - knobBoxW * 0.5f,
                               area.getY() + 26.0f, knobBoxW, knobBoxH);
    secondaryKnob1.paramID = config.secParam1ID;

    if (config.index == 0)
      secondaryKnob1.label = "TONE";
    else
      secondaryKnob1.label = "RATE";
  }
}

static void drawMiniArcKnob(juce::Graphics &g, juce::Rectangle<float> b,
                            const juce::String &label, float value01,
                            juce::Colour color) {
  float knobDiameter = juce::jmin(b.getWidth() * 0.70f, b.getHeight() - 14.0f);
  float radius = knobDiameter * 0.5f;
  float centreX = b.getCentreX();
  float centreY = b.getY() + radius + 1.0f;
  ...
  juce::Path trackPath;
  trackPath.addCentredArc(centreX, centreY, radius, radius, 0.0f, startAngle,
                          endAngle, true);
  ...
  if (value01 > 0.01f) {
    juce::Path fillPath;
    fillPath.addCentredArc(centreX, centreY, radius, radius, 0.0f, startAngle,
                           currentAngle, true);
    g.setColour(color);
  ...
  g.fillEllipse(centreX - radius * 0.5f, centreY - radius * 0.5f, radius,
                radius);
  ...
  // Space Grotesk sub-label positioned cleanly directly beneath the knob with dedicated vertical spacing
  g.setFont(SpectralUILookAndFeel::getSpaceGrotesk(8.5f, false));
  g.setColour(juce::Colour(0x8A, 0x8E, 0x9E));
  float lblY = centreY + radius + 3.0f;
  juce::Rectangle<float> lblRect(b.getX(), lblY, b.getWidth(), 12.0f);
  g.drawText(label, lblRect.toNearestInt(), juce::Justification::centred,
             false);
}
...
void EffectsPanel::EffectZoneComponent::paint(juce::Graphics &g) {
  ...
  float ledX = headerRect.getX() + 3.0f;
  ...
  // Effect Name (JetBrains Mono 11.2f bold)
  g.setFont(SpectralUILookAndFeel::getJetBrainsMono(11.2f, true));
  if (isExpanded)
    g.setColour(config.lcdColor.brighter(0.5f));
  else if (enabled)
    g.setColour(isHeaderHovered ? juce::Colour(0xDD, 0xDB, 0xE4)
                                : juce::Colour(0x9E, 0x9E, 0xAC));
  else
    g.setColour(isHeaderHovered ? juce::Colour(0x8A, 0x8E, 0x9A)
                                : juce::Colour(0x56, 0x58, 0x64));

  juce::Rectangle<float> nameRect(ledX + ledSize + 3.0f, headerRect.getY(),
                                  headerRect.getRight() - (ledX + ledSize + 4.0f),
                                  headerRect.getHeight());
  g.drawText(config.name, nameRect.toNearestInt(),
  ...
  g.setColour(isMixBarHovered ? config.lcdColor.withAlpha(0.35f)
                              : juce::Colour(0x18, 0x1A, 0x24));
  ...
  if (fillW > 0.5f) {
    ...
    juce::ColourGradient grad(botCol, trackX, trackY, topCol, trackX + fillW,
                              trackY, false);
    ...
    if (enabled) {
      ...
      g.setColour(config.lcdColor.brighter(0.5f));
      g.fillRoundedRectangle(markX, trackY - 1.0f, 2.0f, trackHeight + 2.0f,
                             1.0f);
    }
  }
  ...
  if (expansionProgress > 0.05f) {
    ...
    g.setColour(enabled ? config.lcdColor.withAlpha(0.20f)
                        : juce::Colour(0x12, 0x13, 0x1A));
    g.fillRoundedRectangle(bypassRect, 2.5f);
    g.setColour(enabled ? config.lcdColor.withAlpha(0.60f)
                        : juce::Colour(0x28, 0x2A, 0x36));
    g.drawRoundedRectangle(bypassRect, 2.5f, 0.7f);

    g.setFont(SpectralUILookAndFeel::getSpaceGrotesk(8.5f, true));
    g.setColour(enabled ? config.lcdColor.brighter(0.4f)
                        : juce::Colour(0x60, 0x64, 0x72));
    g.drawText(enabled ? "ACTIVE" : "BYPASS", bypassRect.toNearestInt(),
    ...
    if (config.index == 3) // DELAY
    {
      ...
      g.setColour(delayIsSynced ? config.lcdColor.withAlpha(0.20f)
                                : juce::Colour(0x12, 0x13, 0x1A));
      g.fillRoundedRectangle(syncRect, 2.5f);
      g.setColour(delayIsSynced ? config.lcdColor.withAlpha(0.60f)
                                : juce::Colour(0x28, 0x2A, 0x36));
      g.drawRoundedRectangle(syncRect, 2.5f, 0.7f);

      g.setFont(SpectralUILookAndFeel::getSpaceGrotesk(8.5f, true));
      g.setColour(delayIsSynced ? config.lcdColor.brighter(0.4f)
                                : juce::Colour(0x60, 0x64, 0x72));
      g.drawText(delayIsSynced ? "SYNC" : "FREE", syncRect.toNearestInt(),
                 juce::Justification::centred, false);
      ...
      if (delayIsSynced) {
        ...
        juce::String divName =
            (delayDivisionIdx >= 0 && delayDivisionIdx < names.size())
                ? names[delayDivisionIdx]
                : "1/8";
        g.drawText("DIV: " + divName, divRect.toNearestInt(),
                   juce::Justification::centred, false);
      } else {
        // Draw Time Knob
  ...
  if (readoutAlpha > 0.01f && readoutText.isNotEmpty()) {
    ...
    g.drawText(readoutText, roRect.toNearestInt(), juce::Justification::centred,
               false);
  }
}
  ...
  if (isExpanded && expansionProgress > 0.5f) {
    ...
    if (config.index == 3) // DELAY
    {
      ...
      if (getDelaySyncToggleBounds().contains(e.position)) {
        ...
        if (onParameterAdjusted)
          onParameterAdjusted(config.index,
                              delayIsSynced ? "SYNC: ON" : "SYNC: OFF");
        resized();
        repaint();
        return;
      }
      ...
      if (!delayIsSynced && secondaryKnob1.bounds.contains(e.position)) {
        ...
        secondaryKnob1.dragStartVal =
            getNormalizedParamValue(secondaryKnob1.paramID);
        return;
      }
      ...
      if (secondaryKnob2.bounds.contains(e.position)) {
        ...
        secondaryKnob2.dragStartVal =
            getNormalizedParamValue(secondaryKnob2.paramID);
        return;
      }
    } else {
      // Tone / Rate Knob
...
void EffectsPanel::EffectZoneComponent::mouseDrag(const juce::MouseEvent &e) {
  ...
  if (secondaryKnob1.isDragging) {
    ...
    float nextNorm =
        juce::jlimit(0.0f, 1.0f, secondaryKnob1.dragStartVal + dy / 100.0f);
  ...
  if (secondaryKnob2.isDragging) {
    ...
    float nextNorm =
        juce::jlimit(0.0f, 1.0f, secondaryKnob2.dragStartVal + dy / 100.0f);
...
void EffectsPanel::EffectZoneComponent::mouseDoubleClick(
...
EffectsPanel::EffectsPanel(juce::AudioProcessorValueTreeState &apvtsRef,
                           std::function<double()> bpmProviderFn)
    : apvts(apvtsRef), bpmProvider(bpmProviderFn) {
  setupZones();
...
void EffectsPanel::setupZones() {
  ...
  const std::vector<EffectZoneConfig> defs = {
      {0, "DRIVE", juce::Colour(0xD9, 0x77, 0x06), "FX_DRIVE_ENABLE",
       "FX_DRIVE_AMOUNT", "FX_DRIVE_TONE", ""},
      {1, "CHORUS", juce::Colour(0x3B, 0x82, 0xF6), "FX_CHORUS_ENABLE",
       "FX_CHORUS_AMOUNT", "FX_CHORUS_RATE", ""},
      {2, "PHASER", juce::Colour(0x8B, 0x7A, 0xA8), "FX_PHASER_ENABLE",
       "FX_PHASER_AMOUNT", "FX_PHASER_RATE", ""},
      {3, "DELAY", juce::Colour(0x14, 0xB8, 0xA6), "FX_DELAY_ENABLE",
       "FX_DELAY_AMOUNT", "FX_DELAY_TIME", "FX_DELAY_FEEDBACK"},
      {4, "GATE", juce::Colour(0x93, 0x33, 0xEA), "FX_SIDECHAIN_ENABLE",
       "FX_SIDECHAIN_MIX", "FX_SIDECHAIN_RATE", ""}};
...
void EffectsPanel::paint(juce::Graphics &g) {
  ...
  juce::ColourGradient bgGrad(topBg, bounds.getX(), bounds.getY(), botBg,
  ...
  g.drawHorizontalLine((int)(bounds.getY() + 1.0f), bounds.getX() + 5.0f,
  ...
  for (int i = 1; i < 5; ++i) {
    ...
    g.drawVerticalLine((int)divX, bounds.getY() + 4.0f,
