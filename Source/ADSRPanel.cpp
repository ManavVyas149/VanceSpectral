...
void ADSRPanelLookAndFeel::drawRotarySlider(juce::Graphics& g, int x, int y, int width, int height,
                                            ...
                                            juce::Slider& slider)
{
    ...
    auto fillGradient = juce::ColourGradient(
        ...
        juce::Colour(160, 80, 255), static_cast<float>(x + width), static_cast<float>(y + height), false);

    // Draw background track arc
    juce::Path backgroundTrack;
    backgroundTrack.addCentredArc(centreX, centreY, radius, radius, 0.0f, rotaryStartAngle, rotaryEndAngle, true);
    g.setColour(outlineColour);
    g.strokePath(backgroundTrack, juce::PathStrokeType(3.5f, juce::PathStrokeType::curved, juce::PathStrokeType::rounded));

    // Draw active value arc track
    if (sliderPos > 0.0f)
    {
        juce::Path valueTrack;
        valueTrack.addCentredArc(centreX, centreY, radius, radius, 0.0f, rotaryStartAngle, angle, true);
        g.setGradientFill(fillGradient);
        g.strokePath(valueTrack, juce::PathStrokeType(3.5f, juce::PathStrokeType::curved, juce::PathStrokeType::rounded));
    }
    ...
    juce::Path p;
    p.addRoundedRectangle(-pointerThickness * 0.5f, -pointerLength, pointerThickness, pointerLength, 1.2f);
...
//==============================================================================
ADSRPanel::ADSRPanel(juce::AudioProcessorValueTreeState& apvts, const juce::String& panelTitle, const juce::String& paramPrefix)
    : title(panelTitle)
{
